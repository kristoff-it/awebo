...
fn capturePush(audio: *Audio, frames: [*]const f32, frame_count: u32) callconv(.c) void {
    ...
    write.packet.silence = silence;
    write.packet.len = audio.capture_encoder.encodeFloat(&pcm, &write.packet.data) catch |err| {
...
const JitterBuffer = struct {
    ...
    pub fn writePacket(jb: *JitterBuffer, io: Io, restart_id: u32, seq: u32, data: []const u8) void {
        if (jb.expected_next_seq == 0) {
            jb.current_restart = restart_id;
            jb.expected_next_seq = seq;
        }

        if (restart_id != jb.stats.restart) {
            jb.stats = .{
            ...
            jb.packets.write(restart_id, seq, data) catch {
    ...
    const PlaybackPacketRing = struct {
        ...
        pub fn write(pb: *PlaybackPacketRing, restart_id: u32, seq: u32, data: []const u8) !void {
            const w = pb.write_index.load(.acquire);
            const r = pb.read_index.load(.acquire);
            if (mask2(w + buffer_len) == r) return error.Full;

            pb.meta[mask(w)] = .{ .restart = restart_id, .seq = seq };
