...
pending_new_server: ?*Core.ui.ChannelCreate = null,
in_progress_host_join: ?Core.ui.FirstConnectionStatus = null,
show_new_host: bool = false,
show_add_host: bool = false,
pending_new_host: ?*Core.ui.ChannelCreate = null,
show_invite_signup: bool = false,
err_msg: ?[]const u8 = null,

pub fn draw(empty: *Empty, core: *Core) !void {
    ...
    if (empty.in_progress_host_join) |*status| {
        if (status.get() == .success) {
            empty.show_new_host = false;
            empty.show_add_host = false;
            empty.in_progress_host_join = null;
        }
    }

    {
        ...
        if (dvui.button(@src(), "Join Existing", .{}, .{})) {
            empty.show_add_host = true;
        }
        if (dvui.button(@src(), "Invite Sign-up", .{}, .{})) {
            empty.show_invite_signup = true;
        }
    }

    if (empty.show_add_host) {
        try empty.newHostFloatingWindow(core);
    } else if (empty.show_invite_signup) {
        try empty.inviteSignUpFloatingWindow(core);
    }
}

fn newHostFloatingWindow(empty: *Empty, core: *Core) !void {
    const gpa = core.gpa;
    ...
    fw.dragAreaSet(dvui.windowHeader("Add new server", "", &empty.show_add_host));
    ...
    dvui.labelNoFmt(@src(), empty.err_msg orelse "", .{}, .{
        .gravity_x = 0.5,
        .expand = .horizontal,
    });
    ...
    if (clicked or enter_pressed) blk: {
        ...
        if (address_trimmed.len == 0) {
            empty.err_msg = "Missing IP Address!";
            return;
        }

        const username_trimmed = std.mem.trim(u8, username, " \t\n\r");
        if (username_trimmed.len == 0) {
            empty.err_msg = "Missing Username!";
            return;
        }

        empty.err_msg = null;
        _ = Io.net.IpAddress.parse(address_trimmed, 1991) catch |err| {
            empty.err_msg = @errorName(err);
            break :blk;
        };

        // TODO: we should not be discarding this.
        empty.in_progress_host_join = .{};
        _ = try core.hostJoin(address_trimmed, username_trimmed, password, &empty.in_progress_host_join.?);
    }
}

fn inviteSignUpFloatingWindow(empty: *Empty, core: *Core) !void {
    const gpa = core.gpa;
    ...
    fw.dragAreaSet(dvui.windowHeader("Invite Sign-up", "", &empty.show_invite_signup));
    ...
    if (clicked or enter_pressed) blk: {
        ...
        if (password.len == 0) {
            empty.err_msg = "Missing Password!";
            return;
        }

        if (!std.mem.eql(u8, password, password_confirm)) {
            empty.err_msg = "Passwords do not match!";
            return;
        }
        ...
        const invite_parsed = awebo.Invite.parse(gpa, invite_trimmed) catch {
            empty.err_msg = "Invalid invite format!";
            break :blk;
        };
