...
pub fn draw(cp: *ChatPanel, core: *Core, frozen: bool) !void {
    ...
    try cp.header(core, h, c);
    try cp.sendBar(core, h, c, frozen);
    try cp.typingActivity(core, h, c);
    ...
    try cp.messageList(core, h, c.id, &c.kind.chat);
    cp.sidebar(core, h);
}

fn header(cp: *ChatPanel, core: *Core, host: *Host, channel: *Channel) !void {
...

    {
        ...
        if (search_bar.enter_pressed) submit: {
            ...
            if (cp.search.state != .none) {
                cp.search.query.deinit(core.gpa);
                if (cp.search.state == .present) {
                    cp.search.reply.deinit(core.gpa);
                }
                cp.search.state = .none;
            }
            ...
            cp.search.query = .{ .origin = origin, .query = query };
        ...
        if (core.search_messages_reply) |smr| {
            ...
            if (cp.search.state != .none and cp.search.origin == smr.origin) {
                cp.search.reply = smr;
                cp.search.state = .present;
            } else {
                smr.deinit(core.gpa);
            }
        }
...
fn typingActivity(cp: *ChatPanel, core: *Core, h: *awebo.Host, c: *Channel) !void {
    _ = cp;
...
fn sendBar(cp: *ChatPanel, core: *Core, h: *awebo.Host, c: *Channel, frozen: bool) !void {
    _ = cp;
...
fn messageList(cp: *ChatPanel, core: *Core, h: *awebo.Host, channel_id: awebo.Channel.Id, c: *Chat) !void {
    _ = cp;
...
fn sidebar(cp: *ChatPanel, core: *Core, host: *Host) void {
    ...
    if (sidebar_state.show_search) {
...

        {
            ...
            if (cp.search.state != .none) {
                ...
                if (cp.search.state == .waiting) {
                    ...
                    const length = std.fmt.bufPrint(&buffer, "{d}", .{cp.search.reply.results.len}) catch unreachable;
                    search_header_search_text.addText(length, .{});
                    search_header_search_text.addText(" results", .{});
                }
                search_header_search_text.addText(" for \"", .{});
                search_header_search_text.addText(cp.search.query.query, .{
                    .font = font.withStyle(.italic).withWeight(.bold),
                });
                search_header_search_text.addText("\"", .{});
                if (cp.search.state == .waiting) search_header_search_text.addText("...", .{});
            }
        }
        ...
        if (cp.search.state == .present) {
            ...
            for (cp.search.reply.results, 0..) |result, idx| {
                ...
                drawMessage(host, result.preview.author, result.preview.created.fmt(&core.tz, host.epoch), .highlighted, result.preview.text, idx);
            }
        }
    }
}
